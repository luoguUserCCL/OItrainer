...
function outingTrainingWithSelection(difficulty_choice, province_choice, selectedNames, inspireTalents){
  ...
  try{
    ...
    if(totalReduction > 0){
      ...
      try{ if(window.pushEvent) window.pushEvent({ name: '研学经费减免', description: `天赋导致集训费用减少 ¥${applied}（来自: ${reductions.map(r=>r.student+':'+'¥'+r.amount).join(', ')}）`, week: game.week }); }catch(e){}
      log(`研学经费减免：共 -¥${applied}（明细: ${reductions.map(r=>r.student+':'+'¥'+r.amount).join(', ')})`);
    }
  }catch(e){ console.error('collect outing cost reductions error', e); }

  if(game.budget < final_cost){ alert("经费不足，无法研学！"); return; }
  game.recordExpense(final_cost, `研学：${target.name}`);
  log(`研学：${target.name} (${target.type})，难度:${difficulty_choice}，参与人数:${participantCount}，费用 ¥${final_cost}`);
  ...
  for(let s of selectedStudents){
    ...
    if(mismatch){
      const message = `这次研学与学生${s.name}实力不匹配，压力增加，收获减少`;
      pushEvent({ name: '研学不匹配', description: message, week: game.week });
    }
  }
  ...
  if(__before && __after) __summarizeSnapshot(__before, __after, `研学：${target.name} 难度${difficulty_choice}`);
}
