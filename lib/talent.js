...
(function(global){
  const TalentManager = {
    ...
    tryAcquireTalent(student, multiplier = 1.0) {
      ...
      return typeof acquiredName !== 'undefined' ? acquiredName : null;
    },
  ...
  checkAndHandleTalentLoss(student) {
    ...
    registerDefaultTalents(game, utils){
      ...
      this.registerTalent({
        ...
        this.registerTalent({
          ...
          handler: function(student, eventName, ctx){
            try{
              ...
              if(student._talent_state['极端偏科']) return null; // only once per contest
              const keys = ['knowledge_ds','knowledge_graph','knowledge_string','knowledge_math','knowledge_dp'];
              ...
              student._talent_state['极端偏科'] = { up: up, down: down };
              return '极端偏科发动：' + up.replace('knowledge_','') + ' +200%，' + down.replace('knowledge_','') + ' -50%（本场临时）';
            }catch(e){ console.error('极端偏科 天赋错误', e); }
            return null;
          }
        });
        ...
        // 劳逸结合：吃饭、娱乐、休息的压力恢复效果翻倍
        this.registerTalent({
          name: '劳逸结合',
          description: '吃饭、娱乐、休息时压力恢复效果翻倍。',
          color: '#66BB6A',
          prob: 0.10,
          kind: 'positive',
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              if(eventName !== 'entertainment_finished' && eventName !== 'rest_finished' && eventName !== 'treat_finished') return null;
              if(!ctx) return null;
              var relief = Number(ctx.pressureRelief) || 0;
              if(relief <= 0) return null;
              // 额外再减少一次相同数值（翻倍效果）
              student.pressure = Math.max(0, Number(student.pressure || 0) - relief);
              return '劳逸结合：' + (eventName === 'treat_finished' ? '吃饭' : eventName === 'rest_finished' ? '休息' : '娱乐') + '减压效果翻倍！';
        ...
        this.registerTalent({
          ...
          handler: function(student, eventName, ctx){
            try{
              // 同时响应 week_start（真实模式）和 week_end（简化模式）
              if(eventName === 'week_start' || eventName === 'week_end'){
                // 直接应用额外压力恢复
