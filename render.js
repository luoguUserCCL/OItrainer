...
function renderAll(){
  ...
  let displayComfort = game.getComfort();
  ...
  for(let s of game.students){
    ...
    let pressureValue = Math.floor(s.pressure);
  ...
  try{
    ...
    if(!raw){ 
      if(typeof game !== 'undefined' && game && game.students) {
        ...
        el.innerText = '无结算记录，无法显示结局。请确保游戏正常结束。调试信息：- 存储中无oi_coach_save数据- 全局game对象不存在或无效'; 
        return; 
      }
    }
    let o;
  ...
  let endingReason = normalizeEndingReason(rawEnding || (o.endingReason || o.oi_coach_ending_reason || '赛季结束'));
    ...
    if(o.students && o.students.length > 0) {
      ...
      for(let s of o.students) {
        ...
        const pressureValue = (s && typeof s.pressure === 'number') ? Math.floor(s.pressure) : '—';
        let talentsHtml = '';
        try{
...
function outingTrainingUI() {
    ...
    $('out-go').onclick = () => {
      ...
      if(!selectedNames || selectedNames.length === 0){ alert('请至少选择一名学生参加研学！'); return; }
